            ...
            <tbody>
                {#each balances as balance}
                    <tr>
                        ...
                        <td>{(parseFloat(balance.buying_liabilities) + parseFloat(balance.selling_liabilities)).toFixed(7)}</td>
                        <td>{balance.limit ?? 'n/a'}</td>
                        <th>
                        {#if balance.asset_type !== 'native'}
                            <form method="POST" action="?/preview" use:enhance>
                                <input type="hidden" name="source" id="source" value={data.bpaPublicKey} />
                                <input type="hidden" name="asset" id="asset" value={`${balance.asset_code}-${balance.asset_issuer}`} />
