...
<form method="POST" action="?/preview" use:enhance>
    ...
    <div class="form-control w-full max-w-xs">
        ...
        <select id="destination" name="destination" class="select select-bordered"
            ...
            on:change={checkDestinationFunded(destinationPublicKey)}
        >
    ...
    {#if createAccount}
        <div class="alert alert-warning shadow-lg">
            <div>
                ...
                <span>Account Not Funded: You are sending a payment to an account that does not yet exist on the Stellar ledger. Your payment will take the form of a <code>creatAccount</code> operation, and the amount you send must be at least 1XLM.</span>
            </div>
        </div>
