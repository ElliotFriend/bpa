<script>
    ...
    import {
        getBalanceHomeDomains,
        getAccountBalances,
        startTransaction,
    } from '$lib/utils/horizonQueries'
    ...
    const transfer = async (direction, assetCode, homeDomain = 'testanchor.stellar.org') => {
        let { id, type, url } = await initiateTransfer(
            $webAuthStore[homeDomain],
            direction,
            homeDomain,
            { asset_code: assetCode, account: publicKey }
        )
        ...
        window.addEventListener(
            ...
            async (event) => {
                ...
                transaction = transaction
                    .addMemo(
                        Memo.hash(
                            Buffer.from(event.data.transaction.withdraw_memo, 'base64').toString(
                                'hex'
                            )
                        )
                    )
    ...
    const auth = async (homeDomain) => {
        ...
        open(
            ...
            {},
            {
                ...
                onClose: () => {
                    console.log(
                        'closing, here is the current error message',
    ...
    const query6 = async (assetCode, homeDomain = 'testanchor.stellar.org') => {
        let { transactions } = await queryTransfers6(
            $webAuthStore[homeDomain],
            assetCode,
            publicKey,
            homeDomain
    ...
    const transfer6 = async (direction, assetCode, homeDomain = 'testanchor.stellar.org') => {
        let transfer6 = await initiateTransfer6(
            $webAuthStore[homeDomain],
            assetCode,
            data.publicKey,
            direction,
    ...
    const deleteCustomer = async (homeDomain = 'testanchor.stellar.org') => {
        let deleteResponse = await deleteSep12Customer(
            $webAuthStore[homeDomain],
            publicKey,
            homeDomain
        )
...
<div class="prose mx-20 my-10">
    ...
    <p>
        Below, are listed all of your trusted assets with the required infrastructure to facilitate
        deposit and/or withdrawals.
    </p>
        ...
        {#each homeDomains as asset}
            ...
            <form
                on:submit|preventDefault={() =>
                    transfer6('deposit', asset.asset_code, asset.home_domain)}
            >
            ...
            <button class="btn" on:click={() => launchTransferModal(asset.home_domain)}
                >Launch Transfer Modal</button
            >
            ...
            <form
                on:submit|preventDefault={() =>
                    transfer('deposit', asset.asset_code, asset.home_domain)}
            >
                <button class="btn-primary btn" type="submit">SEP-24 Deposit</button>
            </form>
            <form
                on:submit|preventDefault={() =>
                    transfer('withdraw', asset.asset_code, asset.home_domain)}
            >
                <button class="btn-primary btn" type="submit">SEP-24 Withdraw</button>
            </form>
